# Interactive Work Sessions — Resilient Multi-Delivery AI Collaboration

**Publication #19 · v1 · February 2026**
**Languages / Langues**: English (this document) | [Français](https://packetqc.github.io/knowledge/fr/publications/interactive-work-sessions/)
...
**Claude** (Anthropic, Opus 4.6) — AI development partner. Co-author and practitioner of these patterns — every session that applies this methodology validates it. The methodology documents itself by being applied during its own creation.
...
| No push until session end | `recover` can't retrieve unpushed work | Common |
...
`session-protocol.md` covers the lifecycle (wakeup → work → save). `interactive-diagnostic.md` covers debugging sessions. But the **during-work resilience patterns** — progressive commits, issue anchoring, context budget management, multiple session types — were practiced implicitly without documentation. After context compaction, these patterns vanished.

---

## The Solution

A formal methodology framework: one umbrella (`methodology/interactive-work-sessions.md`) plus dedicated per-type files that codify resilient interactive session patterns across five session types.
...
All types inherit the same resilience framework. The phases are templates — real sessions blend types naturally. A documentation session may discover a bug (→ diagnostic), which reveals a missing feature (→ development). Three types have dedicated methodology files; the remaining two are documented in the umbrella methodology.
...
|---------|--------------|----------|-------------|
| **Git branch** | `recover`, `resume`, manual PR | Session crash, context overflow | Never committed |
...
Todo 3 → work → commit → [CRASH]
                           ↓
                    New session:
                    recover → retrieves todos 1 + 2 + 3 (if pushed)
                    issue  → shows what todo 3 was doing
                    resume → if checkpoint exists, restarts todo 3
```

**The gap progressive commits close**: Without intermediate pushes, a crash after 4 of 5 todos loses ALL work. With progressive pushes, only the in-progress todo is at risk.
...
- "That's behavior by the methods applied by knowledge" → the meta-methodology proving its own need
...
This extends universal inheritance to the platform layer — when something is discovered in a session, it persists in the issue even if the session crashes before committing.
...
| Session types not differentiated | Five types with phase templates |
...
| Session crash + push | `recover` in new session | At most current in-progress todo |
| Session crash, no push | Issue has context + manual recovery | Uncommitted work only |
| API 400 (unrecoverable) | New session + `recover` + issue | Uncommitted since last push |
...
6. **The methodology proves itself** — this publication was created in an interactive documentation session that applied its own patterns
...
|---|-------------|-------------|
| 3 | [AI Session Persistence](../ai-session-persistence/) | Foundational persistence methodology |
| 8 | [Session Management](../session-management/) | Lifecycle commands (wakeup, save, resume, recover, recall) |
| 11 | [Success Stories](../success-stories/) | Validated session outcomes |
| 18 | [Documentation Generation](../documentation-generation/) | Universal inheritance principle |
| 14 | [Architecture Analysis](../architecture-analysis/) | System architecture context |
