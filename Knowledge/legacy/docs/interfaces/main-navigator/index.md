...
<div class="chrome-bar" id="chrome-bar">
  <span class="chrome-toggle" id="chrome-toggle" title="Collapse/expand header">▲</span>
  <span class="orient-toggle" id="orient-toggle" title="Toggle page orientation">
    <a data-orient="portrait" class="orient-active">Portrait</a>
    <a data-orient="landscape">Landscape</a>
  </span>
  <span class="lang-toggle">
    <a href="{{ '/interfaces/main-navigator/' | relative_url }}" class="lang-active">EN</a>
    <a href="{{ '/fr/interfaces/main-navigator/' | relative_url }}">FR</a>
  </span>
</div>
...
(function() {
  ...
  function applyChromeState() {
    if (chromeCollapsed) {
      ...
      chromeToggle.title = 'Expand header';
    } else {
      container.classList.remove('chrome-collapsed');
      chromeToggle.textContent = '▲';
      chromeToggle.title = 'Collapse header';
    }
  }
  applyChromeState();
  ...
  document.querySelectorAll('.chrome-bar .lang-toggle a:not(.lang-active)').forEach(function(langLink) {
    langLink.addEventListener('click', function() {
      ...
      var targetPath = BASE + '/fr' + afterBase;
      localStorage.setItem('navigator-center-url-fr', targetPath);
    });
  });

  /* ─── Center panel — always landscape ─── */
  if (centerIframe) {
    centerIframe.addEventListener('load', function() {
      try {
        centerIframe.contentWindow.postMessage({ type: 'set-orientation', orient: 'landscape' }, '*');
        /* Also set localStorage key inside iframe for persistence */
        var cPath = '';
        try { cPath = centerIframe.contentWindow.location.pathname.replace(/\/+$/, ''); } catch(ex) {}
        if (cPath) {
          var cKey = 'knowledge-orient:' + cPath;
          try { centerIframe.contentWindow.localStorage.setItem(cKey, 'landscape'); } catch(ex) {}
        }
      } catch(e) {}
    });
  }
...
(function() {
  ...
  /* Inject panel radio group before the PDF button in the layout toolbar */
  ...
  panelGroup.innerHTML =
    '<label class="pub-export-radio" title="Export center panel">' +
      '<input type="radio" name="navPanel" value="center"' + (selectedPanel === 'center' ? ' checked' : '') + '> Center' +
    '</label>' +
    '<label class="pub-export-radio" title="Export right panel">' +
      '<input type="radio" name="navPanel" value="right"' + (selectedPanel === 'right' ? ' checked' : '') + '> Right' +
    '</label>';
  ...
  /* Override PDF button — print selected panel's iframe */
  ...
  newPdf.addEventListener('click', function(e) {
    ...
    try {
      ...
      /* Inject cover page if the iframe doesn't have one */
      if (!fdoc.getElementById('pub-cover-page')) {
        ...
        cover.innerHTML =
          '<div class="cover-body">' +
            ...
            '<div class="cover-gen-line">Generated: ' + gen + '</div>' +
          '</div>';
        /* Add print styles for the cover */
  ...
  /* Override DOCX button — trigger export in selected panel's iframe */
...
(function() {
  ...
  /* Theme selector — propagate changes to iframes */
  ...
  /* Page size radios — propagate changes to iframes */
  ...
  /* Orientation radios — also propagate to both iframes (not just right) */
  ...
  /* On iframe load — push current theme + page size + orientation */
