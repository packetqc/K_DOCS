...
description: "Tableau de bord vivant suivant le réseau de connaissances distribuées : version du cerveau maître, statut des projets satellites, dérive de version, connaissances récoltées et publications découvertes. Mis à jour à chaque exécution de harvest."
...
> **Publication parente** : [#4 — Connaissances distribuées]({{ '/fr/publications/distributed-minds/' | relative_url }}) — l'architecture que ce tableau de bord visualise
...
Les assistants de codage IA acquièrent une mémoire persistante via `CLAUDE.md` et `notes/` — mais chaque projet évolue indépendamment. Le **système de Knowledge distribuées** les connecte : un cerveau maître pousse la méthodologie vers les satellites au démarrage, et `harvest` ramène les connaissances évoluées.

Cette publication est un **tableau de bord vivant** — mis à jour à chaque exécution de `harvest`. C'est la conscience de soi du réseau.

## Réseau satellite

> Mis à jour à chaque exécution de `harvest`. N'inclut que les dépôts que l'utilisateur possède et auxquels Claude Code a reçu accès — aucun dépôt externe ou tiers.
...
| Composant | Rôle |
|-----------|------|
| **Cerveau maître** (`packetqc/knowledge`) | Dépôt central : méthodologie, patterns, leçons, publications |
| **Satellites** (dépôts projets) | Projets indépendants qui héritent et font évoluer les connaissances |
| **Push** (wakeup) | Les satellites lisent le maître au démarrage — « mettre les lunettes » |
| **Harvest** (entrant) | Le maître parcourt les branches satellites, extrait les connaissances évoluées |
...
|--------|-----------|---------|
| **Core** (CLAUDE.md) | Stable | Identité, méthodologie, évolution |
| **Prouvé** (patterns/, lessons/) | Validé | Éprouvé à travers les projets |
...
```
Note de session → flag harvest → minds/<projet>.md → validé entre projets → connaissance core
```

Les découvertes migrent vers le haut à travers les couches en prouvant leur valeur à travers plusieurs projets. Le tableau de bord suit cette progression en temps réel.
