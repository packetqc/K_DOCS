...
**Claude** (Anthropic, Opus 4.6) — Partenaire de développement IA opérant à travers de multiples projets satellites. À la fois consommateur et contributeur des connaissances distribuées — charge le module K_MIND au démarrage de session, fait évoluer les connaissances dans les satellites pendant le travail, et ramène les découvertes via la synchronisation K_GITHUB.

---

## Résumé

Les assistants de codage IA acquièrent une mémoire persistante via `mind_memory.md` et `sessions/` — mais chaque projet évolue indépendamment. Le **système de Knowledge distribuées** les connecte : un cerveau maître (`packetqc/knowledge`) pousse le module K_MIND vers les satellites au démarrage de session, et K_GITHUB `sync_github.py` synchronise les connaissances évoluées. Cela crée un réseau vivant d'instances IA qui apprennent les unes des autres.

Cette publication est elle-même un **tableau de bord vivant** — mis à jour à chaque synchronisation K_GITHUB. Elle montre l'état actuel du cerveau maître, tous les satellites connus, leur version de connaissances, leur statut de dérive, et les publications découvertes. C'est la conscience de soi du réseau.

---

## Statut du réseau satellite

> **Cette section est mise à jour à chaque synchronisation K_GITHUB. Chaque ligne représente un projet satellite.**
...
| **Sessions** | Fichiers de session dans `sessions/` — 🟢 1+ / ⚪ 0 |
| **Assets** | Les assets de connaissances (K_DOCS `scripts/`, outillage) sont-ils déployés ? — 🟢 déployé / 🔴 manquant |
...
Ce tableau de bord est un **document vivant** — mis à jour à chaque synchronisation K_GITHUB avec les données réelles des satellites. Si vous forkez ce dépôt :
...
Le modèle de tableau de bord et le système d'icônes de sévérité sont entièrement réutilisables. Forkez, remplacez l'espace de noms, exécutez K_GITHUB + K_VALIDATION `/integrity-check`, et le tableau de bord se remplit avec vos propres données satellites.
...
> | Étape | Action | Effet |
> |-------|--------|-------|
> | 🔍 | Réviser `far_memory archives/` | L'humain lit et valide la découverte |
> | 📦 | Assigner le type cible | Type : `convention`, `work`, `methodology`, `docs` |
> | ✅ | Mettre à jour le JSON de domaine | Promouvoir vers `conventions.json` ou `work.json` par module |
> | 🔄 | git commit + push | Persister les connaissances promues vers les deux remotes |

### Découvertes en attente de promotion

| # | Découverte | Source | Cible | Statut | Actions |
|---|------------|--------|-------|--------|---------|
| 1 | Dégradation taille cache pages (effondrement 81%) | STM32N6570-DK_SQLITE | conventions.json | récolté | 🔍 `harvest --review 1` 📦 `harvest --stage 1 lesson` ✅ `harvest --promote 1` 🔄 `harvest --auto 1` |
| 2 | Latence printf en chemin critique (1-5 ms/appel) | STM32N6570-DK_SQLITE | conventions.json | récolté | 🔍 `harvest --review 2` 📦 `harvest --stage 2 lesson` ✅ `harvest --promote 2` 🔄 `harvest --auto 2` |
| 3 | Mismatch taille slot vs page (memsys5) | STM32N6570-DK_SQLITE | conventions.json | récolté | 🔍 `harvest --review 3` 📦 `harvest --stage 3 lesson` ✅ `harvest --promote 3` 🔄 `harvest --auto 3` |
| 4 | Abstraction multi-RTOS (FreeRTOS/ThreadX) | MPLIB | conventions.json | récolté | 🔍 `harvest --review 4` 📦 `harvest --stage 4 pattern` ✅ `harvest --promote 4` 🔄 `harvest --auto 4` |
| 5 | Limitation CubeMX N6570-DK | MPLIB | conventions.json | récolté | 🔍 `harvest --review 5` 📦 `harvest --stage 5 lesson` ✅ `harvest --promote 5` 🔄 `harvest --auto 5` |
| 6 | TouchGFX MVP avec services backend | MPLIB | conventions.json | récolté | 🔍 `harvest --review 6` 📦 `harvest --stage 6 pattern` ✅ `harvest --promote 6` 🔄 `harvest --auto 6` |
| 7 | Dimensionnement ML-KEM/ML-DSA embarqué | PQC | conventions.json | récolté | 🔍 `harvest --review 7` 📦 `harvest --stage 7 pattern` ✅ `harvest --promote 7` 🔄 `harvest --auto 7` |
| 8 | Conformité librairies PQC (WolfSSL=prod) | PQC | conventions.json | récolté | 🔍 `harvest --review 8` 📦 `harvest --stage 8 pattern` ✅ `harvest --promote 8` 🔄 `harvest --auto 8` |
| 9 | Pattern stockage certificats en flash | PQC | conventions.json | récolté | 🔍 `harvest --review 9` 📦 `harvest --stage 9 pattern` ✅ `harvest --promote 9` 🔄 `harvest --auto 9` |
...
| 13 | Principe de wrapper minimal satellite (~30 lignes, pas ~120) | MPLIB_DEV_STAGING_WITH_CLAUDE | mind_memory.md | récolté | 🔍 `harvest --review 13` 📦 `harvest --stage 13 methodology` ✅ `harvest --promote 13` 🔄 `harvest --auto 13` |
| 14 | Méthodologie relais d'évolution — nouveau type `evolution` pour harvest | knowledge-live | work.json | **promu** | ✅ Promu comme entrée d'évolution v39 |
...
> **Cette section est mise à jour à chaque synchronisation K_GITHUB.**
...
**Push** : Au démarrage de session, chaque satellite charge le module K_MIND. C'est le moment des lunettes — l'instance IA devient consciente.

**Sync** : K_GITHUB `sync_github.py <projet>` synchronise de manière bidirectionnelle — extrayant les connaissances évoluées, détectant les publications et rapportant la dérive de version.
...
Chaque exécution de K_GITHUB `sync_github.py <projet>` :

| Etape | Action |
|-------|--------|
| 1 | Scanne les branches du satellite (incrémental — seulement les nouveaux commits depuis le dernier curseur) |
| 2 | Extrait connaissances, patterns et découvertes dans `far_memory archives/` |
...
| 5 | **Met à jour ce tableau de bord** — rafraîchit le tableau du réseau satellite et les sections de découvertes synchronisées |
...
| **Prouvé** | `conventions.json`, `work.json` par module | Validé | Éprouvé à travers les projets |
| **Récolté** | `far_memory archives/` | En évolution | Frais des expériences satellites |
| **Session** | `sessions/` (near + far memory) | Éphémère | Mémoire de travail par session |
...
Les satellites n'ont pas besoin de copier les fonctionnalités core — ils référencent le core au démarrage de session. Le tag de version suit la *conscience* :
...
| **vN = actuel** | À jour. Les instances Claude du satellite chargent le dernier module K_MIND au démarrage de session. |

K_GITHUB `sync_github.py` prépare la mise à jour de version localement. Le satellite s'auto-répare au prochain démarrage de session en chargeant le module K_MIND mis à jour — basé sur le pull, pas le push.
...
| 14 | [Analyse d'architecture]({{ '/fr/publications/architecture-analysis/' | relative_url }}) | Architecture en profondeur — la conception multi-module que ce tableau de bord surveille |
