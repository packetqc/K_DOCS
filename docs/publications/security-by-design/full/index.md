...
> **Parent publication**: [#0 — Knowledge]({{ '/publications/knowledge-system/' | relative_url }}) | **Summary**: [#9 — Security by Design]({{ '/publications/security-by-design/' | relative_url }})
...
| [Proxy Architecture](#proxy-architecture) | Container proxy boundaries and access control |
| [Owner-Scoped Operations](#owner-scoped-operations) | Namespace isolation for all operations |
| [Audit Methodology](#audit-methodology) | How security audits are performed |
...
| Component | Why no credentials needed |
|-----------|--------------------------|
| **Git operations** | Use public HTTPS URLs — no SSH keys, no tokens in URLs |
| **Claude Code authentication** | Handled by the proxy — the repo never sees auth tokens |
| **GitHub API** | Uses `gh` CLI which authenticates via the user's local config, not stored tokens |
| **No external services** | No API keys, no webhook secrets, no database credentials |
...
| **Namespace change** | Forker replaces `packetqc` with their username to use the system for their own projects |
...
| **Publications** | Technical papers with full content | Intentionally public — published for sharing |
...
| Internal IPs or hostnames | Should not be in public files |
| Email addresses | Beyond published contact info |
| Webhook URLs with tokens | Should not be committed |
...
|------|-----------------|
| Push to assigned task branch | Succeeds |
...
|-----------|-------------|
| **Credentials are unnecessary, not scrubbed** | The architecture never requires storing credentials. This is stronger than "we deleted the tokens" — there were never tokens to delete. |
| **The namespace is the boundary** | Security comes from owner-scoping, not from secrets. Changing the GitHub username redirects the entire system. |
| **Public by default** | Every component is designed to be safely public. Private data stays in the user's local environment, never in the repo. |
| **Defense in depth** | Seven independent layers. Failure of any single layer doesn't compromise security. |
| **Audit-friendly** | The entire security posture can be verified with standard git tools and regex pattern scanning. |
| **Fork-friendly** | The system is designed to be forked. A forker gets methodology and tooling, not access. |
...
| Level | Architecture discovery |
|-------|----------------------|
| **Level 0** | Proxy reality (v17) — Claude Code sessions run behind a proxy that scopes push access to the assigned task branch only |
| **Level 1** | Ephemeral token protocol (v27) — Fine-grained PAT with read-only Contents scope enables `git clone` of private repos |
| **Level 2** | API bypass (v28) — The two-channel model: git through proxy (restricted), GitHub REST API direct (unrestricted with token). Confirmed minimum: `Contents: RW` + `Pull requests: RW` + `Projects: RW` + `Metadata: Read` (mandatory) — 4 permissions total |
| **Level 3** | Classic PAT with full `repo` scope — unnecessarily broad, violates least privilege |

### Level 2 — Confirmed Minimum Permissions

Validated on 2026-02-21 via full autonomous cycle (create PR #137 + merge via API):
...
| **Projects: Read and write** | **YES** | Create and manage GitHub Project boards |
| **Metadata: Read-only** | **YES** (mandatory) | Auto-included by GitHub — cannot be unchecked |
| Issues | No | Not used by autonomy cycle |
| Actions | No | Not used by autonomy cycle |
| Webhooks | No | Not used by autonomy cycle |
| Pages | No | Not used by autonomy cycle |
| Administration | No | Not needed |

Only **4 permissions** required (3 selected + 1 mandatory) for full knowledge autonomy.

### Principle of Least Privilege

Always use the lowest level that meets the session's needs:
...
The token is **not encrypted in memory** after reception. In-memory encryption without hardware backing (HSM/TPM/SGX) is security theater — the decryption key must coexist in the same process memory.

**Future**: knowledge-live PQC project targets hardware-backed key storage (STM32 secure element). With a hardware-held key, the token could be encrypted in RAM with genuine protection — the key never enters process memory.

### Phase 3 — Usage

| Operation | Exposure | Mitigation |
|-----------|----------|------------|
| `curl -H "Authorization: token <T>"` | `/proc/<pid>/cmdline` | Container isolation |
| `git clone https://<T>@github.com/...` | Process list | Container isolation + prefer headers |
| GitHub REST API over HTTPS | None | TLS 1.3 encryption in transit |

### Phase 4 — Disposal

| Material | Disposal method | When |
|----------|----------------|------|
| Context window | Destroyed by Anthropic runtime | Session end |
| Python variable | Garbage collected (not zero-overwritten) | Scope exit |
| Key material (`/dev/shm/`) | Zero-overwrite + unlink | `destroy()` |
| Image file | `rm -f` immediately | After extraction |

**Known gap**: Python strings are immutable — GC does not zero memory before freeing (Python bug #17405, open since 2013). Token may persist in freed pages. **Planned improvements**: store tokens in `bytearray` (mutable, can be zeroed in-place), add `mlock()` to prevent swap exposure (FIPS 140-3, HashiCorp Vault pattern), `prctl(PR_SET_DUMPABLE, 0)` to prevent core dumps, explicit zeroing before dereference (NIST SP 800-57 §8.3.4).

### Phase 5 — Post-Session Verification

Verify no token residue: no files contain `github_pat_`, no git history contains it, no environment variables. Container teardown destroys all process memory.
...
| **NIST SP 800-88** | Media sanitization (applied to RAM) | ⚠️ Partial — keys yes, token pending |
...
| **pyATS secret_strings** pattern | Encrypted at-rest for persistent secrets | N/A — token never at rest |
...
|---|-------------|-------------|
| 0 | [Knowledge]({{ '/publications/knowledge-system/' | relative_url }}) | Parent — the system this publication secures |
| 3 | [AI Session Persistence]({{ '/publications/ai-session-persistence/' | relative_url }}) | Session notes security model |
| 4 | [Distributed Minds]({{ '/publications/distributed-minds/' | relative_url }}) | Harvest security scope |
| 4a | [Knowledge Dashboard]({{ '/publications/distributed-knowledge-dashboard/' | relative_url }}) | Dashboard data exposure model |
