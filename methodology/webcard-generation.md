...
| **Size** | 1200 x 630 px |
| **Format** | Animated GIF |
| **Themes** | 2 themes — Cayman (light) + Midnight (dark) |
| **Languages** | EN + FR (4 GIFs total per card: 2 themes × 2 languages) |
...
Examples:
- `behavioral-intelligence-en-daltonism-light.gif`
- `knowledge-2-fr-daltonism-dark.gif`
- `live-mindmap-en-daltonism-light.gif`

---

## Theme Convention — Two Themes Only

By convention, only **two themes** are used for webcard generation:

| Theme | Role | Background |
|-------|------|------------|
| **Daltonism Light** | Light theme | `#faf6f1` warm cream to `#f0e8dc` warm beige |
| **Daltonism Dark** | Dark theme | `#1a1a2e` purple-navy to `#141428` deeper purple-navy |

These themes provide **higher contrast ratios** and **warmer tones** than Cayman/Midnight, designed for color vision accessibility (deuteranopia/protanopia safe). The asset footprint is 4 GIFs per publication (2 themes × 2 languages).

### Color Palettes

| Color | Daltonism Light | Daltonism Dark |
|-------|----------------|----------------|
| `BG_TOP` | `#faf6f1` warm cream | `#1a1a2e` purple-navy |
| `BG_BOT` | `#f0e8dc` warm beige | `#141428` deeper purple-navy |
| `ACCENT_BLUE` | `#0055b3` high-contrast blue | `#5b9bd5` muted blue |
| `TEXT_WHITE` | `#1a1a2e` dark text | `#e8e0d4` warm light text |
| `TEXT_MUTED` | `#5c5c78` | `#8888a0` |
| `BOX_BG` | `#faf6f1` warm cream | `#232337` dark fill |
...
```bash
node Knowledge/K_DOCS/scripts/capture_mindmap.js cayman          # Light theme, normal mode
node Knowledge/K_DOCS/scripts/capture_mindmap.js midnight --full  # Dark theme, full mode
```
...
```bash
python3 Knowledge/K_DOCS/scripts/stitch_webcard.py cayman
python3 Knowledge/K_DOCS/scripts/stitch_webcard.py midnight
```
...
<meta property="og:image" content="https://site/assets/og/slug-lang-daltonism-light.gif">
...
Webcards are assets bundled with publications. After generating a webcard, ensure the parent publication follows the **Publication Completion Checklist** in `methodology/documentation-generation.md#publication-completion-checklist` — the `og_image` field in front matter, the HTML redirect files with OG tags, and the link registry entry all depend on the webcard being in place.
...
- `methodology/documentation-generation.md` — Front matter contract and publication completion checklist
